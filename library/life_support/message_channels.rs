pub fn InitializeMessageChannels(mut config: *mut VLMConfig) {
    ...
    let mut p: *mut EmbMessageChannel = &mut *(EmbCommAreaPtr as *mut EmbWord).offset(
        cp
    ...
    *fresh3 = &mut *(EmbCommAreaPtr as *mut EmbWord).offset(
        (*p).guestToHostQueue
    ) as *mut EmbWord as PtrV as *mut EmbQueue;
    (*(*cc).guestToHostQueue).signal = InstallSignalHandler(
        ::std::mem::transmute::<Option<fn(*mut EmbCommandChannel) -> ()>, ProcPtrV>(
            Some(ExecuteGuestCommands as fn(*mut EmbCommandChannel) -> ())
        ),
        cc as PtrV,
        false
    );
    ...
    *fresh4 = &mut *(EmbCommAreaPtr as *mut EmbWord).offset(
...
pub fn PollMessageChannels() {
    ...
    messageChannel = &mut *(EmbCommAreaPtr as *mut EmbWord).offset(
        (*EmbCommAreaPtr).command_channel
    ) as *mut EmbWord as PtrV as *mut EmbMessageChannel;
    while !messageChannel.is_null() {
        subtypeData = ((((*messageChannel).subtypeData0 as u64) << 32) |
            (*messageChannel).subtypeData1) as PtrV as *mut EmbMessageSubtypeData;
        guestToHostQueue = &mut *(EmbCommAreaPtr as *mut EmbWord).offset(
            (*messageChannel).guestToHostQueue
        ) as *mut EmbWord as PtrV as *mut EmbQueue;
...
fn ThreadActiveMessageChannel(mut theChannel: *mut EmbMessageChannel) {
    ...
    let mut subtypeData: *mut EmbMessageSubtypeData = 0 as *mut EmbMessageSubtypeData;
    let ref mut fresh6 = (*(
        ((((*theChannel).subtypeData0 as u64) << 32) |
            (*theChannel).subtypeData1) as PtrV as *mut EmbMessageSubtypeData
        ...
        if commandPtr != 0 {
            ...
            match (*command).header.opcode() {
                1 => {
                    ...
                    mbinChannel = &mut *(EmbCommAreaPtr as *mut EmbWord).offset(
...
pub fn ResetMessageChannel(mut channel: *mut EmbChannel) {
    ...
    allocatedByVLM = ((
        (channel as *mut EmbWord).offset_from(EmbCommAreaPtr as *mut EmbWord) as EmbPtr
    ) > (*EmbCommAreaPtr).host_buffer_start + (*EmbCommAreaPtr).host_buffer_size) as bool;
    ...
    *fresh16 = 0 as *mut EmbMessageChannel;
    match (*messageChannel).subtype {
        2 => {
            ...
            ResetOutgoingQueue(
                &mut *(EmbCommAreaPtr as *mut EmbWord).offset(
                    (*messageChannel).guestToHostReturnQueue
                ) as *mut EmbWord as PtrV as *mut EmbQueue
            );
        }
        3 => {
            ...
            ResetIncomingQueue(
                &mut *(EmbCommAreaPtr as *mut EmbWord).offset(
                    (*messageChannel).hostToGuestSupplyQueue
                ) as *mut EmbWord as PtrV as *mut EmbQueue
            );
            ResetOutgoingQueue(
                &mut *(EmbCommAreaPtr as *mut EmbWord).offset(
                    (*messageChannel).hostToGuestQueue
                ) as *mut EmbWord as PtrV as *mut EmbQueue
            );
