...
block of CDR+Tag, then 4 blocks of immediate data. This is instead of storing 8 bytes
where only 5 would used.
:::
...
ignored (except by the garbage collector, which expects it to be `cdr-nil`). The address in the
...
(make-array 4 :displaced-to a :dispaced-index-offset 10. :element-type '(unsigned-byte 4)))
```

Software defines the precise algorithm to be used
...
4, Read the memory ‘word at `(+ base-address (]sh subscript (- %%byte-packing)))`

5. Use the low-order bits of the subscript, `%%byte-packing`, and `%%element-type`
...
`dtp-call-compiled-even`/`dtp-call-compiled-odd` is used, inter-function references bypass. the
...
- Packed instructions –- data types with type codes 60-77 are used for words that
...
starting function calls. In addition, data type `dtp-external-value-cell-pointer` (type code 4) is used to fetch the contents of
...
high level application, copy-array-portion or bitbit. Reading a `BAR` that was
